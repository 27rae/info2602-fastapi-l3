...
def toggle_todo(todo_id:int, username:str):
    ...
    pass
## if command executed multiple times = uncheck and checks
## if ID set to todo that does not exist = prints this todo doesnt exist
## if change username to a user that does not exist
...
def list_todos():
    with get_session() as db:
        todos =db.exec(select(Todo)).all() #this takes every todo row
        for todo in todos: #then this loops
